...
function LoadFuclty(element) {
    ...
    $.ajax({
        type: "GET",
        url: '/Authors/GetFuclty?instid=' + instid + '&departid=' + departid,
...
function renderFuclty() {
    ...
    $ele.append($('<option/>').val('0').text('اختر الكلية'));
...
function rendeDepartment() {
    ...
    $ele.empty();
    $ele.append($('<option/>').val('0').text('اختر القسم'));
...
$(document).ready(function () {
    ...
    $('#addinstitfac').click(function () {
        ...
        if (isAllValid) {
            ...
            if (ele) {
                swal("تحذير", "هذا الحقل موجود بالفعل", "warning");
                return;
            }
    ...
    $('#instfac').on('click', '.remove', function () {

       

        //if (document.getElementsByName("mycontainer").length == 0) {

        //    document.getElementById("btnsubmit").disabled = true;
        //}
        $(this).parents('tr').remove();
        if ($('#instfac tr').length == 0) {
            //document.getElementById("btnsubmit").disabled = true;
